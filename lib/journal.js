...
function articleHtml(a) {
  if (!a || !a.url) return '';
  const title = esc(a.title || '头条文章');
  const body = a.text ? `<div class="article-text">${decorate(a.text)}</div>` : '';
  return `<div class="article">📄 <a href="${esc(a.url)}" target="_blank" rel="noopener">${title}</a>${body}</div>`;
}
...
.article{margin-top:8px;background:#faf5ea;border-radius:6px;padding:8px 12px;font-size:14px}
.article-text{margin-top:8px;background:#fff;border:1px dashed #e0d5c0;border-radius:6px;padding:10px 14px;font-size:13.5px;line-height:1.9;max-height:340px;overflow-y:auto}
...
@media print{
  ...
  .article-text{max-height:none!important;border:none;padding:6px 0}
...
(function(){
  ...
  async function ensureImgsForPrint(){
    ...
    var B=250;
    for(var i=0;i<imgs.length;i+=B){
      var slice=imgs.slice(i,i+B);
      slice.forEach(function(im){ im.loading='eager'; });
      await Promise.all(slice.map(function(im){
  ...
  if(pb)pb.onclick=async function(){ pb.disabled=true; await ensureImgsForPrint(); pb.disabled=false; window.print(); };
