  ...
  const steps = {
    ...
    async mediaPhase() {
      ...
      while (cp.media.idx < queue.length && started < 3) {
        ...
        if (existing && existing.ok && existing.blob && existing.blob.size > 0) continue;
        try {
      ...
      if (cp.media.idx < queue.length) return true;
      // 队列跑完后做一次完整性对账：逐条核对本地库（含空 blob 损坏记录），缺失的自动补齐
      if (!cp.media.verified) {
        cp.media.verified = true;
        await saveCheckpoint(cp);
        let stored = 0, healed = 0, still = 0;
        for (const item of queue) {
          let ok = false;
          try {
            const rec = await getMedia(item.url);
            ok = !!(rec && rec.ok && rec.blob && rec.blob.size > 0);
          } catch (e) { /* 视为缺失 */ }
          if (ok) { stored++; continue; }
          try {
            await downloadMedia(item);
            healed++;
            if (item.kind === 'video') stats.videosOk++;
        ...
        stats.verifyExpected = queue.length;
        stats.verifyStored = stored;
        stats.verifyHealed = healed;
        stats.verifyMissing = still;
        dirtyStats = true;
        await persistStats();
        if (still > 0) recordError(stats, `完整性对账后仍缺媒体 ${still} 个（多为线上已删除，保留链接）`);
      }
      return false;
    },
