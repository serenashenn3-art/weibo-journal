...
function extractPageInfo(mblog) {
  ...
  if (pi.type === 'article' || pi.type === 'webpage') {
    return { article: { title: pi.page_title || '', url: pi.page_url || '' } };
  }
  return {};
}
...
async function loadStats() {
  return (await getMeta('stats')) || {
    ...
    videosOk: 0, videosFail: 0, likesNew: 0, errors: [],
  };
}
...
export async function runCrawl({ shouldStop, onProgress }) {
  const options = (await getMeta('options')) || { video: true, likes: true, comments: true };
  ...
  const mediaSkip = new Set((await getMeta('mediaSkip')) || []);
    ...
    async mediaPhase() {
      ...
      while (cp.media.idx < queue.length && started < 3) {
        ...
        try {
          ...
          if (item.kind === 'video') stats.videosOk++; else stats.imagesOk++;
        } catch (e) {
          if (item.kind === 'video') stats.videosFail++; else stats.imagesFail++;
          ...
          await setMeta('mediaSkip', [...mediaSkip]);
...
async function buildMediaQueue(uid) {
  ...
  for (const p of all) {
    ...
    if (p.video && p.video.url) q.push({ url: p.video.url, mid: p.mid, kind: 'video', ts: p.created_at });
